...
def main():
    ...
    # 如果存在 edge_split 且包含 valid，则构造验证集（临时副本，避免修改原训练邻居映射）
    valid_loader = None
    if hasattr(dataset_for_lm, 'edge_split') and dataset_for_lm.edge_split is not None and 'valid' in dataset_for_lm.edge_split:
        # 复制一份 dataset_for_lm，然后把 gnid2neighbors_train 改为仅使用 valid 对
        val_tag = copy.copy(dataset_for_lm)
        # 构建 gnid2neighbors_valid：每个源节点映射到其 valid 目标列表
        gnid2neighbors_valid = {gnid: [] for gnid, _ in enumerate(val_tag.data_list)}
        valid_srcs = dataset_for_lm.edge_split['valid']['source_node']
        valid_dsts = dataset_for_lm.edge_split['valid']['target_node']
        for src, dst in zip(valid_srcs, valid_dsts):
            gnid2neighbors_valid[src].append(dst)
        val_tag.gnid2neighbors_train = gnid2neighbors_valid

        valid_dataset = CGTPDataset(
            get_text,
            args.num_neighbor,
            val_tag,
            args.text_encoder_name,
    ...
    cgtp_model = CGTPModel(
        text_encoder_name=args.text_encoder_name,
        text_encoder_is_pretrained=True,  # 可根据需要添加参数
        text_encoder_num_layers_to_train=args.text_encoder_num_layers_to_train,
        text_embed_dim=args.text_emb_dim,
    ...
    for epoch in range(args.num_epochs):
        ...
        # 使用验证集损失驱动 ReduceLROnPlateau（若没有验证集则回退到 avg_loss）
        if valid_loader is not None:
            val_loss = validate_epoch(cgtp_model, valid_loader, device)
            try:
                lr_scheduler.step(val_loss)
            except TypeError:
                lr_scheduler.step()
            print(f"使用验证损失调度 lr： val_loss={val_loss:.4f}")
        else:
            try:
                lr_scheduler.step(avg_loss)
            except TypeError:
                lr_scheduler.step()
            print(f"使用训练损失调度 lr： train_loss={avg_loss:.4f}")

        # 打印当前学习率（可能有多个 param_group）以便调试
